...
synth = 
   let oct = 8
       ...
       in mix $ loopBy iter $ atSco razorLeadFast
                           $ mel $ (++ [rest $ tick 4]) $ zipWith str (map tick 
                                                         [6,2,2,2,11
                                                         ,1,5,1,5,1,8
                                                         ,2,2,7,2,2,2,11
                                                         ,1,4,1,5,1,8]) 
                                         $ map ins [0.10,1.00,1.01,1.03,1.05
...
canon = 
   let oct = 8
       ...
       tick n = 60 * 16 / 96 * n / pace
       in mix $ loopBy iter $ atSco razorLead
                           $ mel $ zipWith str (map tick 
                                              [ 5,1,5,1,5,1,5,1,5,1,5,1,12
                                              , 5,1,5,1,5,1,5,1,5,1,5,1,12
                                              ]) 
...
pad =                                                    
   let oct = 7
       ins pch = temp (0.08, cpspch $ oct + pch)
       tick n = 60 * 16 / 16 * n / pace
       in mix $ loopBy iter $ atSco polySynth
                   $ har [ mel $ zipWith str (map tick [1,1,2,1,1,2,1,1,2,1,1,2 ]) 
                                       $ map ins 
                              [ 0.10,1.03,1.05
                              , 1.04,1.00,1.05
                              , 0.10,1.01,1.05
                              , 1.04,1.00,1.05
                                 ...
                                 , mel $ zipWith str (map tick [2,2,2,2,4,2,2])
                                  $ map ins 
